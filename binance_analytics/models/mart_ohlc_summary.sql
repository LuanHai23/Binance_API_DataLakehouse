...
WITH candles AS (
    SELECT
        ...
        ROUND(
            (ABS(close_price - open_price) / NULLIF(open_price, 0) * 100)::numeric,
            4
        ) AS body_size_pct,

        ROUND(
            (buy_volume_taker / NULLIF(total_volume, 0) * 100)::numeric,
            2
        ) AS buy_pressure_pct,
...
hourly_agg AS (
    SELECT
        ...
        SUM(buy_volume_taker) OVER (
            PARTITION BY symbol, candle_hour
        ) AS hourly_buy_vol,

        SUM(sell_volume_maker) OVER (
            PARTITION BY symbol, candle_hour
        ) AS hourly_sell_vol,
...
SELECT
    ...
    ROUND(
        (h.hourly_buy_vol / NULLIF(h.hourly_volume, 0) * 100)::numeric,
        2
    ) AS hourly_buy_pct
